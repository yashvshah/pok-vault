...
export class MiddlewareService {
  ...
  async fetchMarketInfoBatch(
  ...
  ): Promise<Map<string, { marketInfo: MiddlewareMarketData; providerId: string; tokenAddress: Address }>> {
    if (tokens.length === 0) {
      return new Map();
    }
    ...
    try {
      ...
      const marketInfoMap = new Map<string, { marketInfo: MiddlewareMarketData; providerId: string; tokenAddress: Address }>();
